...
# last token time is the start of the current epoch. by dividing by WEEK first, since using uints, the decimals get cut off
...
def __init__(
    ...
    _start_time: uint256,
    _token: address, # this is the SCREAM token for us
    _admin: address,
...
def _checkpoint_token():
    ...
    # this is the last snapshot of reward token balance. 
    # this would seem to mean that if tokens aren't claimed in a week they get redistributed to everyone else?
    to_distribute: uint256 = token_balance - self.token_last_balance
    self.token_last_balance = token_balance

    t: uint256 = self.last_token_time # last time we updated our token time
    since_last: uint256 = block.timestamp - t # how long has it been since this epoch started
    self.last_token_time = block.timestamp # update our last token time to be the current timestamp. this is the only place this is updated
    this_week: uint256 = t / WEEK * WEEK # this is the start of the current epoch
    next_week: uint256 = 0

    for i in range(20): # this can catch us up if we're up to 20 weeks behind?
        next_week = this_week + WEEK # the start of the next epoch
        if block.timestamp < next_week: # if we haven't entered our new epoch yet
            ...
            break
        else: # if we have entered our new epoch
